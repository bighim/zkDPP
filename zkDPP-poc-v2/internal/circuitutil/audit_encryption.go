...
// AssertMasterEncrypted is the M1 v2 relation. The calling Event Circuit fixes
// the plaintext shape and public statement; L and n are deliberately absent.
func AssertMasterEncrypted(api frontend.API, pk auditcrypto.PublicKey, message []frontend.Variable, r frontend.Variable, r1 ed.Point, data []frontend.Variable) error {
	if len(message) == 0 || len(message) != len(data) {
		return fmt.Errorf("audit encryption vector shape mismatch")
	}
	if err := auditcrypto.ValidatePoint(pk.Point); err != nil {
	...
	api.AssertIsEqual(r1.Y, expected.Y)
	z := curve.ScalarMul(pub, r)
	k, err := Hash(api, auditcrypto.AuditKeyTag, z.X, z.Y)
	if err != nil {
		return err
	}
	for j := range message {
