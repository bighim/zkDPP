...
func TestHashIsDeterministic(t *testing.T) {
	info := document.DocumentInfo{ProductName: "Aluminum", LotID: "LOT-M1-001", Unit: "kg"}
...
func TestUnitIsBound(t *testing.T) {
	a, err := document.Hash(document.DocumentInfo{ProductName: "Aluminum", LotID: "LOT-M1-001", Unit: "kg"})
	if err != nil {
		t.Fatal(err)
	}
	b, err := document.Hash(document.DocumentInfo{ProductName: "Aluminum", LotID: "LOT-M1-001", Unit: "piece"})
	if err != nil {
		t.Fatal(err)
	}
	if a.Equal(&b) {
		t.Fatal("Unit was not bound to DocumentHash")
	}
}
