...
type ExternalKeyPackage struct {
	Profile      string
	SessionID    string
	Threshold    uint8
	MemberCount  uint8
	PublicKey    PublicKey
	PublicShares [3]Point
}
...
func masterKey(z Point) fr.Element {
	return zkhash.Hash(AuditKeyTag, z.X, z.Y)
}
...
// EncryptWithMasterPublicKey is the v2 relation. Event shape and public values
// are bound by the calling Circuit, so the M6-B1 context and length inputs are
// intentionally absent.
func EncryptWithMasterPublicKey(pk PublicKey, message []fr.Element, r *big.Int) (Ciphertext, error) {
	if len(message) == 0 {
		return Ciphertext{}, fmt.Errorf("empty message")
	}
	if err := ValidatePoint(pk.Point); err != nil {
...
func lagrangeCoefficient(id, other uint8) (*big.Int, error) {
	if id < 1 || id > 3 || other < 1 || other > 3 || id == other {
		return nil, fmt.Errorf("invalid interpolation IDs")
	}
	q := Order()
	den := big.NewInt(int64(id) - int64(other))
	den.Mod(den, q)
	inverse := new(big.Int).ModInverse(den, q)
	if inverse == nil {
		return nil, fmt.Errorf("invalid interpolation denominator")
	}
	lambda := new(big.Int).Mul(big.NewInt(-int64(other)), inverse)
	return lambda.Mod(lambda, q), nil
}

// RecoverMasterKey intentionally reconstructs the epoch secret after a
// threshold release. M1 assumes the shares came from an external DKG.
func RecoverMasterKey(pkg ExternalKeyPackage, released []Share) (*big.Int, error) {
	if pkg.Profile != Profile || pkg.SessionID == "" || pkg.Threshold != 2 || pkg.MemberCount != 3 {
		return nil, fmt.Errorf("invalid external key package")
	}
	if err := ValidatePoint(pkg.PublicKey.Point); err != nil {
		return nil, err
	}
	if len(released) != 2 || released[0].ID == released[1].ID {
		return nil, fmt.Errorf("exactly two distinct shares required")
	}
	g := Generator()
	for _, share := range released {
		if err := validateShare(share); err != nil {
			return nil, err
		}
		var public Point
		public.ScalarMultiplication(&g, share.Value)
		if !public.Equal(&pkg.PublicShares[share.ID-1]) {
			return nil, fmt.Errorf("share does not match public share")
		}
	}
	q := Order()
	out := new(big.Int)
	for i, share := range released {
		lambda, err := lagrangeCoefficient(share.ID, released[1-i].ID)
		if err != nil {
			return nil, err
		}
		term := new(big.Int).Mul(lambda, share.Value)
		out.Add(out, term)
	}
	out.Mod(out, q)
	if err := ValidateScalar(out, true); err != nil {
		return nil, err
	}
	var public Point
	public.ScalarMultiplication(&g, out)
	if !public.Equal(&pkg.PublicKey.Point) {
		out.SetInt64(0)
		return nil, fmt.Errorf("recovered master key does not match public key")
	}
	return out, nil
}

func PackageFromShares(sessionID string, pk PublicKey, shares [3]Share) (ExternalKeyPackage, error) {
	if sessionID == "" {
		return ExternalKeyPackage{}, fmt.Errorf("empty session ID")
	}
	if err := ValidateCommittee(pk, shares); err != nil {
		return ExternalKeyPackage{}, err
	}
	pkg := ExternalKeyPackage{Profile: Profile, SessionID: sessionID, Threshold: 2, MemberCount: 3, PublicKey: pk}
	g := Generator()
	for i, share := range shares {
		pkg.PublicShares[i].ScalarMultiplication(&g, share.Value)
	}
	return pkg, nil
}

func DecryptWithMasterKey(master *big.Int, ct Ciphertext) ([]fr.Element, error) {
	if err := ValidateScalar(master, true); err != nil {
		return nil, err
	}
	if len(ct.Data) == 0 {
	...
	var z Point
	z.ScalarMultiplication(&ct.R1, master)
	k := masterKey(z)
