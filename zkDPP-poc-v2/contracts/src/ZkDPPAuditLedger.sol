...
// M7 main path; M6 remains an unchanged baseline.
contract ZkDPPAuditLedger {
...
IZkVerifier public immutable auditExitVerifier;
 ...
 error InvalidEventKind();  error UnknownPolicyFamily();  error PreviousVersionNotRegistered();  error UnknownPolicyReservation();  error InvalidLeafIndex();
...
auditExitVerifier=IZkVerifier(verifierAddresses[7]);
 ...
 function _eventKind() private pure returns(uint8){
  if(msg.sig==ZkDPPAuditLedger.entry.selector)return 0;
  if(msg.sig==ZkDPPAuditLedger.transfer.selector)return 1;
  if(msg.sig==ZkDPPAuditLedger.proceed.selector)return 2;
  if(msg.sig==ZkDPPAuditLedger.recall.selector)return 3;
  if(msg.sig==ZkDPPAuditLedger.merge.selector)return 4;
  if(msg.sig==ZkDPPAuditLedger.split.selector)return 5;
  if(msg.sig==ZkDPPAuditLedger.process.selector)return 6;
  if(msg.sig==ZkDPPAuditLedger.exit.selector)return 7;
...
if(kind==7)return auditExitVerifier;
  revert InvalidAuditShape();
 }
 ...
 function exit(bytes calldata proof,uint256 root,uint256 nf,AuditCipher calldata a) external returns(uint256 aid){
  if(!noteTree.acceptedRoots[root])revert InvalidRoot();_active(NOTE,nf);
  uint256[] memory p=new uint256[](2);p[0]=root;p[1]=nf;
  aid=_complete(proof,p,_one(nf),new ObjectRef[](0),a);emit NoteExited(nf);
 }
