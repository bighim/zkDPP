...
contract ZkDPPStatusLedger {
    ...
    uint256 public constant EMPTY_STATUS_ROOT =
        22537072579266811521376377138270460403410619672927721005324315591145912158653;
    uint256 public constant POLICY_REF_TAG =
        51780253709110118800953810799850621917828020824397005216617999075326973004822;
    ...
    IZkVerifier public immutable statusPrivateSpendVerifier;
    IZkVerifier public immutable statusTransferVerifier;
    IZkVerifier public immutable statusProceedVerifier;
    IZkVerifier public immutable statusRecallVerifier;
    IZkVerifier public immutable statusMergeVerifier;
    IZkVerifier public immutable statusSplitVerifier;
    IZkVerifier public immutable statusProcessVerifier;
    IZkVerifier public immutable statusUpdateVerifier;
    ...
    uint256 public noteStatusRoot = EMPTY_STATUS_ROOT;
    uint256 public voucherStatusRoot = EMPTY_STATUS_ROOT;
    ...
    constructor(
        address statusAuthority_,
        address entryVerifier_,
        address statusPrivateSpendVerifier_,
        address statusTransferVerifier_,
        address statusProceedVerifier_,
        address statusRecallVerifier_,
        address statusMergeVerifier_,
        address statusSplitVerifier_,
        address statusProcessVerifier_,
        address statusUpdateVerifier_,
        address fieldHasher_
    ) {
        if (statusAuthority_ == address(0)) revert InvalidAccount();
        address[9] memory codeAddresses = [
            entryVerifier_,
            statusPrivateSpendVerifier_,
            statusTransferVerifier_,
            statusProceedVerifier_,
            statusRecallVerifier_,
            statusMergeVerifier_,
            statusSplitVerifier_,
            statusProcessVerifier_,
            statusUpdateVerifier_
        ];
        for (uint256 i = 0; i < codeAddresses.length; i++) {
            if (codeAddresses[i].code.length == 0) revert InvalidAccount();
        }
        if (fieldHasher_.code.length == 0) revert InvalidAccount();
        admin = msg.sender;
        statusAuthority = statusAuthority_;
        entryVerifier = IZkVerifier(entryVerifier_);
        statusPrivateSpendVerifier = IZkVerifier(statusPrivateSpendVerifier_);
        statusTransferVerifier = IZkVerifier(statusTransferVerifier_);
        statusProceedVerifier = IZkVerifier(statusProceedVerifier_);
        statusRecallVerifier = IZkVerifier(statusRecallVerifier_);
        statusMergeVerifier = IZkVerifier(statusMergeVerifier_);
        statusSplitVerifier = IZkVerifier(statusSplitVerifier_);
        statusProcessVerifier = IZkVerifier(statusProcessVerifier_);
        statusUpdateVerifier = IZkVerifier(statusUpdateVerifier_);
    ...
    function exit(bytes calldata proof, uint256 noteRoot, uint256 nf) external {
        ...
        _verify(statusPrivateSpendVerifier, proof, _three(noteRoot, noteStatusRoot, nf));
    ...
    function transfer(
        ...
        inputs[0] = noteRoot;
        inputs[1] = noteStatusRoot;
        inputs[2] = nf;
        inputs[3] = rvNew;
        inputs[4] = cmChange;
        inputs[5] = transferEpoch;
        inputs[6] = deltaEpoch;
        _verify(statusTransferVerifier, proof, inputs);
        noteNullifiers[nf] = true;
    ...
    function proceed(bytes calldata proof, uint256 voucherRoot, uint256 rvnf, uint256 cmReceiver)
        external
    {
        _resolutionPreconditions(voucherRoot, rvnf, cmReceiver);
        _verify(
            statusProceedVerifier, proof, _four(voucherRoot, voucherStatusRoot, rvnf, cmReceiver)
        );
    ...
    function recall(
        ...
        inputs[0] = voucherRoot;
        inputs[1] = voucherStatusRoot;
        inputs[2] = rvnf;
        inputs[3] = cmReturn;
        inputs[4] = epoch;
        _verify(statusRecallVerifier, proof, inputs);
        voucherNullifiers[rvnf] = true;
        commitments[cmReturn] = true;
        _append(noteTree, cmReturn, false);
    }

    function merge(bytes calldata proof, uint256 noteRoot, uint256 nf1, uint256 nf2, uint256 cmOut)
        external
    {
        if (!noteTree.acceptedRoots[noteRoot]) revert InvalidRoot();
        ...
        inputs[3] = nf2;
        inputs[4] = cmOut;
        _verify(statusMergeVerifier, proof, inputs);
    ...
    function split(
        ...
        if (!noteTree.acceptedRoots[noteRoot]) revert InvalidRoot();
        if (noteNullifiers[nf]) revert SpentNullifier();
        if (cmOut1 == cmOut2 || commitments[cmOut1] || commitments[cmOut2]) {
        ...
        inputs[3] = cmOut1;
        inputs[4] = cmOut2;
        _verify(statusSplitVerifier, proof, inputs);
        noteNullifiers[nf] = true;
    ...
    function process(
        bytes calldata proof,
        uint256 policyRef,
        uint256 policyScopeRef,
        uint256 noteRoot,
        uint256[3] calldata nf,
        uint256[2] calldata cmOut
    ) external {
        ...
        if (
            record.eventKind != PROCESS || record.inputArity != 3 || record.outputArity != 2
                || record.verifierRef != address(statusProcessVerifier)
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            for (uint256 j = 0; j < i; j++) {
                if (nf[i] == nf[j]) revert DuplicateInput();
            }
        }
        if (cmOut[0] == cmOut[1] || commitments[cmOut[0]] || commitments[cmOut[1]]) {
            revert DuplicateCommitment();
        }
        uint256[] memory inputs = new uint256[](9);
        inputs[0] = policyRef;
        inputs[1] = policyScopeRef;
        inputs[2] = noteRoot;
        inputs[3] = noteStatusRoot;
        inputs[4] = nf[0];
        inputs[5] = nf[1];
        inputs[6] = nf[2];
        inputs[7] = cmOut[0];
        inputs[8] = cmOut[1];
        _verify(statusProcessVerifier, proof, inputs);
        for (uint256 i = 0; i < 3; i++) {
            noteNullifiers[nf[i]] = true;
        }
        for (uint256 i = 0; i < 2; i++) {
            commitments[cmOut[i]] = true;
            _append(noteTree, cmOut[i], false);
        }
    }
    ...
    function updateStatus(
        ...
        if (!_allowedStatus(oldStatus, newStatus)) revert InvalidStatusTransition();
        uint256 oldRoot;
        if (objectType == NOTE) {
            if (index >= noteTree.leafCount || noteTree.nodes[_nodeIndex(0, index)] != objectId) {
                revert InvalidObjectBinding();
            }
            oldRoot = noteStatusRoot;
        } else if (objectType == VOUCHER) {
            if (
                index >= voucherTree.leafCount
                    || voucherTree.nodes[_nodeIndex(0, index)] != objectId
            ) revert InvalidObjectBinding();
            oldRoot = voucherStatusRoot;
        } else {
            revert InvalidObjectType();
        }
        ...
        inputs[4] = oldStatus;
        inputs[5] = newStatus;
        _verify(statusUpdateVerifier, proof, inputs);
        if (objectType == NOTE) noteStatusRoot = newRoot;
        else voucherStatusRoot = newRoot;
        emit StatusChanged(objectType, objectId, index, oldStatus, newStatus, newRoot);
    }
    ...
    function registerPolicy(
        ...
        if (
            reservation.eventKind == PROCESS
                && (inputArity != 3
                    || outputArity != 2
                    || verifierRef != address(statusProcessVerifier))
        ) revert InvalidPolicyRecord();
    ...
    function _allowedStatus(uint8 oldStatus, uint8 newStatus) private pure returns (bool) {
        return oldStatus == ACTIVE && newStatus == FROZEN || oldStatus == FROZEN
            && newStatus == ACTIVE || oldStatus == FROZEN && newStatus == REVOKED;
    }
    ...
    function _verify(IZkVerifier verifier, bytes calldata proof, uint256[] memory inputs)
        private
        view
    {
