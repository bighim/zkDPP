...
> zkDPP는 물품의 질량·탄소·재활용 State를 숨긴 채 공급망 Event의 계산 일관성을 증명하고, 승인된 Policy·동결·비공개 backward audit·DPP Claim을 함께 제공하는 시스템입니다.
...
각 물품의 State를 `(질량, 탄소, 재활용 원료)` 순서로 적습니다.
...
공정에서 질량 20이 감소하고 탄소 30이 추가되며 재활용 원료량은 변하지 않는다고 가정합니다.

```text
입력 합계       = (320, 230, 30)
공정 변화       = (-20, +30,  0)
공정 후 전체 값 = (300, 260, 30)
```

공정 결과를 주제품과 폐기물로 나눕니다.

```text
주제품 D = (270, 260, 30)  → ELIGIBLE
폐기물 W = ( 30,   0,  0)  → WASTE
            ----------------
출력 합계 = (300, 260, 30)
```
...
| $a_{\mathrm{rec}}$ | 물품에 귀속된 재활용 원료 질량 또는 credit입니다. |
...
`DocumentHash`는 Note의 State와 별도로 물품 설명에 binding하는 값입니다.
...
policyRecords[policyRef]
  → 이 Policy는 어떤 Event용이고 어떤 VK를 사용하는가?
...
현재 상세 명세의 $M_{\max}$, $N_{\max}$와 padding은 이전 구조를 설명합니다. 상세 구현 명세는 Policy별 exact $m$-to-$n$ 구조에 맞게 별도 수정해야 합니다.

> **미확정:** Factory가 Policy Authority에게 제출할 Setup 요청의 형식과 근거 문서 범위는 아직 정하지 않았습니다.
...
$$
h=H(d,\mathrm{issuePolicyRef},\mathrm{claimNonce})
$$
...
Verifier는 DPP에서 $h$를 다시 계산하고, 온체인에 등록됐으며 현재 Active인지 확인합니다. Claim은 owner와 nullifier를 가진 소유형 token이 아니라 공개 검증 식별정보입니다.

> **미확정:** DPP core schema, canonicalization, 저장·배포 주체, `DocumentHash`와 $d$의 관계를 정해야 합니다.
...
```text
1. Factory·Prover: off-chain proof 생성
...
이 구간은 Factory가 부담합니다. 입력 수가 늘면 path 검증과 parent encryption이 늘어나므로 proof 생성시간과 메모리가 증가합니다. 이 비용 자체는 Contract gas가 아닙니다.
...
Auditor가 요청한 encryptedParents
  → Custodian마다 자신의 key share로 partial decryption 생성
  → Partial-decryption share를 Auditor에게 전달
```
...
```text
4. Auditor·Status Authority: off-chain graph 복원
...
Auditor는 record 조회, share 요청·수집, share 검증·결합, parent graph 탐색을 부담합니다. 이 구간은 정상 transaction마다 발생하지 않고 감사할 때만 발생합니다.
...
| 측정 주체 | 실제로 측정할 값 |
|---|---|
| Prover | Constraint 수, witness 생성시간, proving time, peak memory, proof bytes |
...
| Auditor·Status Authority | 방문 AuditRecord 수, share 수집시간, share 검증·combine 시간, 전체 audit latency |

### 정상 Event에서 공통으로 발생하는 비용

| 언제 발생하는가? | 처리하는 객체·작업 | 부담 주체 | Big-O |
|---|---|---|---:|
| 기존 객체 $m$개를 소비할 때 | [Membership path 검증](#cost-proof-generation) | Prover | $O(mD)$ |
| 기존 객체 $m$개를 소비할 때 | Status path 검증 | Prover | StatusTree 구현 후 확정 |
| 기존 객체 $m$개를 소비할 때 | [Nullifier 저장](#cost-process-sstore) | Router Contract | $O(m)$ |
| 실제 parent $m$개를 기록할 때 | [Parent encryption](#cost-threshold-encryption) | Prover | Threshold Encryption 방식에 따라 결정 |
...
| Audit 실행 | [Share 검증·결합과 graph 탐색](#cost-audit) | Auditor·Status Authority | $O(KV)$ |
| 새 Policy 등록 | [Circuit Setup·PK·VK·Verifier 등록](#cost-policy-arity) | Policy Authority·Factory | Policy 수 $P$에 대해 $O(P)$ |
...
| 공개 Claim handle의 강한 접근 통제 | 공개된 $h$를 누가 열람하거나 복사할 수 있는지 통제하지 않습니다. 다른 DPP에 재사용하는 것은 binding 검증으로 막습니다. |
...
이 비용은 **스마트 컨트랙트 gas가 아니라 Factory 또는 Prover가 오프체인에서 부담하는 계산 비용**입니다. Circuit constraint가 늘어나면 proof 생성시간, CPU 사용량과 메모리가 증가합니다.
...
현재 합의는 `poc-v2`와 같이 Contract가 Tree node를 직접 저장하는 방식입니다. Output 하나를 append할 때 leaf부터 depth 32의 root까지 node를 갱신하고 다음 상태도 기록합니다.
...
#### Auditor·Status Authority

Auditor는 $V$개의 AuditRecord를 조회하고, 각 record마다 $K$개의 share를 수집·검증·결합합니다.
...
따라서 Auditor의 주요 계산·통신량도 $O(KV)$로 요약할 수 있습니다. 이 비용은 off-chain 비용이며 온체인 gas가 아닙니다. Provenance graph가 깊거나 parent가 많아 방문 record 수 $V$가 커질수록 감사 시간이 증가합니다.
