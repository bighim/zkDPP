...
\begin{itemize}
  ...
  \item \path{benchmarks/anvil-e2e-time.json}: live proof를 포함한 E2E 시간. 본 보고서의 기존
        표는 historical 5회 측정이며, 이후 재현 실행은 1회 정책을 사용한다.
\end{itemize}
...
본 보고서에서 기존에 반복 측정한 시간 지표는 4.2의 historical clean Anvil 5회 E2E 결과를
사용한다. 이후 재현 실행은 1회만 수행한다.
...
receipt 수신까지 수행한 기존 historical clean Anvil 5회 측정의 median이다.
...
생성한 기존 historical clean Anvil 5회의 median이며, 마지막 열은 동일한 5회의 min--max다.
...
\begin{enumerate}
  ...
  \item \textbf{E2E 시간은 proof 생성이 지배한다.}
        ...
        전체 E2E 846.296ms였다.
        기존 E2E 결과는 historical clean Anvil 5회 median과 min/max를 함께 기록했다.
        이후 재현 실행은 1회만 수행하며 세 통계 필드에는 동일한 값이 기록된다.
\end{enumerate}
...
make benchmark-anvil-e2e ANVIL_PORT=18545
make benchmark-anvil ANVIL_PORT=18545
\end{verbatim}
