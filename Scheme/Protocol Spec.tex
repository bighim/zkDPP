...
\begin{tabular}{lll}
\hline
Event & Private input witness & Public protocol object \\
\hline
Entry & 없음 & $cm_{new}$ \\
Transfer & $cm_{old},deadline_{epoch}$ &
$rt,nf,rv_{new},cm_{change},transfer_{epoch},\Delta_{epoch}$ \\
Proceed & $rv_{old},deadline_{epoch}$ & $rvrt,rvnf,cm_{new}$ \\
Recall & $rv_{old},deadline_{epoch}$ &
$rvrt,rvnf,cm_{new},current_{epoch}$ \\
Merge & $cm_{old,1},cm_{old,2}$ & $rt_1,rt_2,nf_1,nf_2,cm_{new}$ \\
Split & $cm_{old}$ & $rt,nf,cm_{new,1},cm_{new,2}$ \\
Process & $(cm_{old,i})_{i=1}^{m}$ &
$(rt_i,nf_i)_{i=1}^{m},(cm_{new,j})_{j=1}^{n}$ \\
Exit & $cm_{old}$ & $rt,nf$ \\
\hline
\end{tabular}
\end{center}

Proceed와 Recall은 $rv_{old}$를 private witness로 소비한다. Public observer는
$o_{rv}$를 모르므로 Transfer가 공개한 $rv$와 resolution transaction의 $rvnf$를
직접 연결할 수 없다.
...
\begin{itemize}
  ...
  \item 각 Process profile은 $1\leq m,n\leq a$를 지원한다. $m,n$은 active input/output
        ...
        별도 public 또는 private enable vector는 두지 않는다. Inactive input의 public
        $rt$, $nf$와 private $cm$, opening, path는 0이고, inactive output의 $cm$과
        $A$ row도 0이다.
  ...
  \item Azeroth와 같은 finalized-note spend unlinkability를 위해 소비하는
        $cm_{old}$는 public statement와 transaction calldata에 포함하지 않고
        private witness로 둔다. Public observer는 $rt$, $nf$와 새 output commitment만
        확인하며, 어떤 과거 $cm$이 소비되었는지는 알 수 없다.
  \item Entry는 소비하는 commitment가 없다. Proceed와 Recall은 finalized $cm$이 아니라
        Recall Voucher $rv$를 private witness로 소비한다. Transfer가 공개한 $rv$와
        resolution transaction의 $rvnf$ 사이의 관계는 secret $o_{rv}$에 의해 숨겨진다.
  \item Input/output commitment edge를 직접 공개하는 provenance event는 두지 않는다.
        Client가 Merkle witness를 구성할 수 있도록 $MT$와 $rvMT$ append의 leaf,
        zero-based index와 new root만 event로 공개한다. 다만 한 transaction이 생성한
        여러 output commitment는 같은 calldata에 함께 나타나므로 서로 함께 생성되었다는
        사실은 공개된다.
\end{itemize}
...
\[
  ...
  \Hash(
    ...
    \addrSender,\addrReceiver,deadline_{epoch},o_{rv}
  )
\]
...
$rvnf=\Hash(o_{rv},rv)$와 $\List{rvnf}$로 관리한다. Proceed와 Recall transaction에는
$rv$를 공개하지 않는다. Public observer는 $o_{rv}$를 모르므로 공개된 $rvnf$를
Transfer transaction의 $rv$와 직접 연결할 수 없다. Sender와 Receiver는 동일한
$o_{rv}$를 공유하므로 같은 resolution identifier를 계산할 수 있다.
...
\begin{enumerate}
  \item public argument, accepted root, output 중복, nullifier와 deadline 같은
        on-chain precondition을 검사한다. 소비하는 $cm_{old}$의 존재 여부와 input
        간 중복은 private witness를 사용하는 Relation 내부에서 검사한다.
  \item ordered public statement를 구성하고 해당 Relation proof를 검증한다.
  \item proof가 유효할 때만 List와 Map을 갱신하고 $MT$ 또는 $rvMT$에 append한다.
\end{enumerate}

Proceed와 Recall의 $rv$는 private witness이므로 Contract는
$rv\in\List{rv}$를 사전 검사하지 않는다. 등록된 Recall Voucher라는 사실은 Circuit의
$rvMT$ membership proof로 검증한다.
...
Sender는 Transfer를 생성할 때 $\Delta_{epoch}$을 선택하고
$deadline_{epoch}=transfer_{epoch}+\Delta_{epoch}$을 계산한다. Transfer Relation은
$deadline_{epoch}$이 이 식을 만족하고 $rv$에 binding되었음을 검증한다. Contract는
public $transfer_{epoch}$이 실행 block의 $\CurrentEpoch()$과 같은지 검사한다.
$deadline_{epoch}$은 Contract storage에 저장하지 않는다.

Recall Relation은 public $current_{epoch}$과 private $deadline_{epoch}$에 대해
$current_{epoch}<deadline_{epoch}$을 검증한다. Contract는 public
$current_{epoch}$이 실행 block의 $\CurrentEpoch()$과 같은지 검사한다. Proceed는
deadline을 검사하지 않으며, 아직 소비되지 않은 Voucher라면 deadline 이후에도 가능하다.

Epoch 값은 Field Element이면서 unsigned 64-bit 정수로 해석한다.

\[
0\leq transfer_{epoch},\Delta_{epoch},deadline_{epoch},current_{epoch}<2^{64}
\]

Transfer Relation의 64-bit range와 deadline equality는 덧셈의 Field wraparound와
unsigned 64-bit overflow를 함께 방지한다.

\[
deadline_{epoch}=transfer_{epoch}+\Delta_{epoch}
\]

$\Delta_{epoch}=0$은 허용한다. 이 경우
$current_{epoch}<deadline_{epoch}$을 만족할 수 없어 Recall은 불가능하다.
...
Finalized note를 소비하는 Transfer, Merge, Split, Process와 Exit에서 membership leaf인
$cm_{old}$와 Merkle path는 private witness다. Contract calldata와 verifier public
statement에는 accepted root와 nullifier만 포함된다. Entry는 input note가 없고,
Proceed와 Recall은 별도 $rvMT$에 저장된 public $rv$를 소비한다.
...
\mathbf{s}_T\gets\mathbf{s}_{in}-\mathbf{s}_C
  \\
transfer_{epoch}\gets\CurrentEpoch()
  \,;\,
deadline_{epoch}\gets transfer_{epoch}+\DeltaEpoch
  \\
...
cm_C\gets
  ...
  \\
\mathbf{x}_T=\{
  rt,nf,rv,cm_C,transfer_{epoch},\DeltaEpoch\}
  \\
\begin{aligned}
  \mathbf{w}_T=\{
    &sk_S,pk_S,pk_R,\\
    &cm_{in},\DocumentHash,q_{in},\mathbf{s}_{in},
    q_T,\mathbf{s}_T,\addrSender,\addrReceiver,deadline_{epoch},\\
...
\mathit{voucher}=\{
  ...
  \\
\mathit{Tx}_T=(
  \pi_T,rt,nf,rv,cm_C,transfer_{epoch},\DeltaEpoch)
  \\
...
\Assert s_{T,k}+s_{C,k}=s_{in,k}
  ...
  \\
\Assert
0\leq transfer_{epoch},\DeltaEpoch,deadline_{epoch}<2^{64}
  \\
\Assert
deadline_{epoch}=transfer_{epoch}+\DeltaEpoch
  \\
\Assert
  rv=
  \Hash(
...
\mathbf{x}_T=\{
  rt,nf,rv,cm_C,transfer_{epoch},\DeltaEpoch\}
  \\
\Assert transfer_{epoch}=\CurrentEpoch()
  \\
...
Proceed는 $rvMT$에 등록된 Recall Voucher를 private witness로 소비한다. Public
statement와 calldata에는 $rv$ 대신 secret-derived nullifier $rvnf$를 포함한다.
...
\begin{gather*}
  ...
  \\
  rvnf\gets\Hash(o_{rv},rv)
  \\
  cm_{recv}
  \gets
  \CM(\DocumentHash,q_T,\mathbf{s}_T,\addrReceiver,o_{recv})
  \\
  \mathbf{x}_P=\{rvrt,rvnf,cm_{recv}\}
  \\
  \mathbf{w}_P=\{
    sk_R,pk_R,rv,
    \DocumentHash,q_T,\mathbf{s}_T,
    \addrSender,\addrReceiver,deadline_{epoch},
  ...
  \\
  \mathit{Tx}_P=(\pi_P,rvrt,rvnf,cm_{recv})
  \\
...
\begin{schemeblock}
\(
  \textsc{ProceedSC}(\pi_P,rvrt,rvnf,cm_{recv})
\)
...
Recall은 $rvMT$에 등록된 Recall Voucher를 private witness로 소비한다. Recall
Relation은 Voucher에 binding된 private $deadline_{epoch}$과 Contract가 제공하는
public $current_{epoch}$을 비교한다.
...
\begin{gather*}
  ...
  \mathrm{GetPath}_{rvMT}(index_{rv})
  \\
  current_{epoch}\gets\CurrentEpoch()
  \\
  ...
  \\
  \mathbf{x}_R=\{rvrt,rvnf,cm_{return},current_{epoch}\}
  \\
  \mathbf{w}_R=\{
    sk_S,pk_S,rv,
    \DocumentHash,q_T,\mathbf{s}_T,
  ...
  \\
  \mathit{Tx}_R=(
    \pi_R,rvrt,rvnf,cm_{return},current_{epoch})
  \\
...
\begin{gather*}
  ...
  \\
  \Assert
  0\leq current_{epoch},deadline_{epoch}<2^{64}
  \,;\,
  \Assert current_{epoch}<deadline_{epoch}
  \\
  \Assert
...
\begin{gather*}
  \mathbf{x}_R=\{rvrt,rvnf,cm_{return},current_{epoch}\}
  \\
  \Assert current_{epoch}=\CurrentEpoch()
  \\
  \Assert rvrt\in\List{rvrt}
  \\
  \Assert rvnf\notin\List{rvnf}
...
cm_{out}\gets
  ...
  \\
\mathbf{x}_M=\{rt_1,rt_2,nf_1,nf_2,cm_{out}\}
  \\
\mathbf{w}_M=\{
    sk,pk,
    \{cm_i,\DocumentHash_i,q_i,\mathbf{s}_i\}_{i=1}^{2},
...
\mathit{note}_{out}=(cm_{out},\DocumentHash_1,q_{out},\mathbf{s}_{out},o_{out})
  \\
\mathit{Tx}_M=(\pi_M,rt_1,rt_2,nf_1,nf_2,cm_{out})
  \\
...
\begin{schemeblock}
\(
  \textsc{MergeSC}(\pi_M,rt_1,rt_2,nf_1,nf_2,cm_{out})
\)
...
cm_{out,j}
  ...
  \quad j\in\{1,2\}
  \\
\mathbf{x}_S=\{rt,nf,cm_{out,1},cm_{out,2}\}
  \\
\mathbf{w}_S=\{
    sk,pk,cm_{in},\DocumentHash,q_{in},\mathbf{s}_{in},
...
\mathit{note}_{out,j}=(cm_{out,j},\DocumentHash,q_j,\mathbf{s}_j,o_j)
  \quad j\in\{1,2\}
  \\
\mathit{Tx}_S=(\pi_S,rt,nf,cm_{out,1},cm_{out,2})
  \\
...
\begin{schemeblock}
\(
  \textsc{SplitSC}(\pi_S,rt,nf,cm_{out,1},cm_{out,2})
\)
...
$rt_i,nf_i$와 private witness의 $cm_i$는 0이고, inactive output의 $cm_j$와
$A$ row도 0이다.
...
\mathbf{x}_{Proc}=\{
    m,n,P_{pub},A^{pad},
    \{rt_i,nf_i\}_{i=1}^{M_{\max}},
    \{cm_{out,j}\}_{j=1}^{N_{\max}}\}
  \\
\begin{aligned}
  \mathbf{w}_{Proc}=\{
    &sk,pk,\\
    &\{cm_{in,i},\DocumentHash_{in,i},q_{in,i},
      \mathbf{s}_{in,i}\}_{i=1}^{M_{\max}},\\
...
\Assert rt_i=nf_i=0
  ...
  \\
\Assert
  cm_{in,i}=\DocumentHash_{in,i}=q_{in,i}=s_{in,i,k}=o_{in,i}=0
...
\text{\bfseries 1. Active array 길이와 public input shape 확인}
  \\
 m\gets\operatorname{len}(\{rt_i\})
  \,;\,
  n\gets\operatorname{len}(\{cm_{out,j}\})
  \\
\Assert m=\operatorname{len}(\{nf_i\})
  \,;\,
  \Assert 1\leq m\leq M_{\max}
  \,;\,
  \Assert 1\leq n\leq N_{\max}
...
\text{\bfseries 2. Fixed-size verifier public input 구성}
  \\
\{rt_i,nf_i\}_{i=1}^{M_{\max}}
  \gets\operatorname{PadZero}_{M_{\max}}(\{rt_i,nf_i\}_{i=1}^{m})
  \\
\{cm_{out,j}\}_{j=1}^{N_{\max}}
...
\mathbf{x}_{Proc}=\{
    ...
    \{cm_{out,j}\}_{j=1}^{N_{\max}}\}
  \\
\text{\bfseries 3. Proof 이전 root와 spent 검사}
  \\
\Assert rt_i\in\List{rt}
  \quad i\in[m]
  \\
...
nf\gets\Hash(sk,cm_{in})
  \\
\mathbf{x}_{Exit}=\{rt,nf\}
  \\
\mathbf{w}_{Exit}=\{
    sk,pk,cm_{in},\DocumentHash,q_{in},\mathbf{s}_{in},addr,path_{in},o_{in}\}
  \\
...
\pi_{Exit}\gets
  ...
  \\
\mathit{Tx}_{Exit}=(\pi_{Exit},rt,nf)
  \\
