...
for profile in poseidon2 sha256; do
  run_dir="$ROOT_DIR/benchmarks/anvil-${profile}-gas-runs"
  mkdir -p "$run_dir"
  cleanup
  ...
  go run ./cmd/evmrun -root "$ROOT_DIR" -backend anvil -rpc "http://127.0.0.1:${ANVIL_PORT}" -chain-id 31337 \
    -receipt-timeout 30s -poll-interval 10ms -profile "$profile" -scenario canonical -threads 8 \
    -fixture "testdata/generated/canonical-${profile}-mt-payloads.json" -run 1 -out "benchmarks/anvil-${profile}-gas-runs/run-01.json"
  GOCACHE="$ROOT_DIR/.cache/go-build" GOMODCACHE="$ROOT_DIR/../poc-v2/.cache/gomod" GOTOOLCHAIN=go1.25.7 \
  go run ./cmd/evmreport -input "benchmarks/anvil-${profile}-gas-runs/run-*.json" -runs 1 \
    -out-json "benchmarks/anvil-${profile}-gas.json" -out-csv "benchmarks/anvil-${profile}-gas.csv"
done
